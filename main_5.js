...
'email': email,'name':name,'lastname':lastname, 'phone': phone,
           ...
           success: function(data) {
            ...
            if (data.data=="confirmation_sent"){
               $(".loader-container_alert").css("display", "none")
              $(".alert_backgraud").css("display", "flex")
            }
            else{
               $(".loader-container_alert").css("display", "none")
            }
...
if (validationResult_name && validationResult_last_name){
  ...
  $(".error_window").css("display", "block")
    ...
    if (String(password_confirm)!=String(password)){
    validation = false
    if (lang_all_page=="ru"){error_text = "Пароли не совподают"}
else if (lang_all_page=="en"){error_text = "Passwords do not match"}
 else if (lang_all_page=="hy"){error_text = "Գաղտնաբառերը չեն համընկնում"}
  $(".error_window").text(error_text)
        $(".error_window").css("display", "block")
    ...
    if (validationLogin && validationPassword && agree.is(':checked') && validationResult_name && validationResult_last_name && phoneInput.isValidNumber() && String(password_confirm)==String(password)){
