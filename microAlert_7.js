...
if(l != ''){
  for (var i = 0, len = l.length; i < len; ++i) {
        ...
        if (parseInt(result['price_pcs']) != 0){
          ...
          price = result['price_size']/10
          if (result['sale_price_size']){
            price = result['sale_price_size']/10
...
$(".header__search__results").append(`<div class="search_item">


                <button type="submit" class=" button button_size-l header__search-btn" style="width:100%;">
       Դիտել բոլորը
...
$(".loader-container_alert").css("display", "block")
             ...
             if (data.data!=0){
              $(".loader-container_alert").css("display", "none")
                 $(".icon.icon-cart .header__control-bage").text(parseInt($(".header__control-bage").text())+1)
                  ...
                  $(".header__control-btn.header__cart .header__control-text").text(parseFloat(data.price_total/pay_rates_search).toFixed(2))
              ...
              if (data.data!=0){
                 ...
                 $(".icon.icon-cart .header__control-bage").text(parseInt($("span.header__control-bage.cart-empty").text())+1)
                  ...
                  if (parseInt(data.min_size) !=  1){
                   $(".header__control-btn.header__cart .header__control-text").text(String(parseFloat(data.price_total/pay_rates_search).toFixed(2))+" "+pay_icon_search)
                 console.log(data.price_total, "data.price_total")
...
function add_favorites(pk, _this, del_favorite=undefined){
  var data = new FormData()
     ...
     $.ajax({
            ...
            success: function(data){
                if (data.data==1){
                    ...
                    $(".navigation-bar__bage").text($("span.header__control-bage.favorites-empty").text())
                }
                else{
                    ...
                    $("span.header__control-bage.favorites-empty").text(parseInt($("span.header__control-bage.favorites-empty").text())-1)
                    $(".navigation-bar__bage").text($("span.header__control-bage.favorites-empty").text())
                    if(del_favorite != undefined){
                      $("#product-preview-elem"+String(del_favorite)).remove()
...
$(".button.button_size-l.favorite_on.favorites_btn").on("click", function(){
        ...
        var name = ""
        var pk = $(this).attr("pk")
        if ($(this).attr("favorte_item_id") != undefined){
          add_favorites(pk, $(this), $(this).attr("favorte_item_id"))
